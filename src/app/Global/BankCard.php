 ...
 * 银行卡名称列表
 */
function GetRBBankNameList()
{
    $bankInfo = RBBankInfo();
    return array_values($bankInfo);
}
 ...
 * 银行卡背景图
 */
function GetRBBankImg($bank)
{
    return "https://apimg.alipay.com/combo.png?d=cashier&t={$bank}";
}

function RBBankCardInfo($cardNum)
{
    $cardType = GetRBBankType();
    $bankName = GetRBBankNameList();
    ...
    if (!$result->validated) {
        ...
        $bankInfo = array(
            ...
            'bankName' => $bankName[$result->bank],   // 银行名称
            'bankImg' => GetRBBankImg($result->bank),
            'cardType' => $result->cardType,         // 银行卡类型, CC 信用卡, DC 储蓄卡
            'cardTypeName' => $cardType[$result->cardType],
        );
    }
